...
# =============================================================================
# Local implementations of functions removed from transformers in v5.x
# =============================================================================


def is_offline_mode() -> bool:
    """
    Check if transformers is running in offline mode.

    Returns:
        `bool`: True if running in offline mode, False otherwise.
    """
    import os

    return os.environ.get("TRANSFORMERS_OFFLINE", "0").lower() in ("1", "true", "yes")
...
def is_safetensors_available() -> bool:
    ...
    Returns:
        `bool`: True if safetensors is available, False otherwise.
    """
    try:
        import safetensors
...
def is_torch_sdpa_available() -> bool:
    """
    Check if torch SDPA (Scaled Dot Product Attention) is available.

    Returns:
        `bool`: True if SDPA is available, False otherwise.
    """
    try:
        import torch
...
def is_remote_url(url_or_filename: str) -> bool:
    ...
    Args:
        url_or_filename (`str`): The string to check.

    Returns:
        `bool`: True if the string is a remote URL, False otherwise.
    """
    ...
    return parsed.scheme in ("http", "https", "ftp", "s3", "gs", "hdfs", "oss")


def download_url(url: str, temp_file) -> None:
    """
    ...
    progress = tqdm(unit="B", total=total, desc=f"Downloading {url}")
...
def id_tensor_storage(tensor: torch.Tensor) -> tuple:
    """
    Return unique identifier to a tensor storage.

    Args:
        tensor (`torch.Tensor`): The tensor to get the storage identifier for.

    Returns:
        `tuple`: A tuple of (device, storage_ptr, storage_size).
    """
    if tensor is None:
        return (None, None, None)
    return (
        tensor.device,
        tensor.storage().data_ptr() if tensor.storage().size() > 0 else 0,
        tensor.storage().size(),
    )
...
def find_pruneable_heads_and_indices(
    ...
    Args:
        heads (`list`): List of heads to prune.
        num_heads (`int`): Number of heads in the model.
        head_size (`int`): Size of each head.
        already_pruned_heads (`set`): Set of already pruned heads.

    Returns:
        `tuple`: A tuple with the remaining heads and the indices of the heads to prune.
    """
    ...
    for head in heads:
        # Compute how many pruned heads are before the head and at the end
        heads_at_the_end = len([h for h in already_pruned_heads if h >= head])
        mask[head - heads_at_the_end] = 0
...
def prune_conv1d_layer(
    ...
    """
    Prune a Conv1D layer to keep only entries in index. A Conv1D works as a Linear layer (see `Conv1D`) but the weights
    are transposed.
    ...
    Returns:
        `nn.Module`: The pruned layer.
    """
    # Conv1D is a custom class from transformers, but we can handle it as a linear layer
    # with transposed weights
    if hasattr(layer, "weight"):
        if dim == 0:
            # Prune the output dimension
            layer.weight = nn.Parameter(layer.weight[index, :])
            if hasattr(layer, "bias") and layer.bias is not None:
                layer.bias = nn.Parameter(layer.bias[index])
        elif dim == 1:
...
def prune_layer(
    ...
    """
    Prune a layer to keep only entries in index.

    Args:
        layer (`nn.Module`): The layer to prune.
        index (`torch.LongTensor`): The indices to keep in the layer.
        dim (`int`, *optional*): The dimension on which to keep the indices. If `None`, will try to guess.
    ...
    if isinstance(layer, nn.Linear):
        ...
        # Generic pruning for layers with weights
        if dim is None:
            dim = 1 if layer.weight.shape[1] > layer.weight.shape[0] else 0
        if dim == 0:
            layer.weight = nn.Parameter(layer.weight[index, :])
            if hasattr(layer, "bias") and layer.bias is not None:
                layer.bias = nn.Parameter(layer.bias[index])
...
def prune_linear_layer(
    ...
    Args:
        layer (`nn.Linear`): The layer to prune.
        index (`torch.LongTensor`): The indices to keep in the layer.
        dim (`int`, *optional*, defaults to 0): The dimension on which to keep the indices.

    Returns:
        `nn.Linear`: The pruned layer.
    """
...
def assert_device_map(device_map: dict, num_layers: int):
    """
    Assert that the device map is valid.

    Args:
        device_map (`dict`): A dictionary mapping layer indices to devices.
        num_layers (`int`): The total number of layers.
    """
    if device_map is None:
        return
    if len(device_map) != num_layers:
        raise ValueError(
            f"The device map should have {num_layers} entries but has {len(device_map)}."
        )
    if not all(isinstance(k, int) for k in device_map.keys()):
        raise ValueError("The keys of the device map must be integers.")


def get_device_map(n_layers: int, devices: list) -> dict:
    """
    Get a balanced device map for model parallelism.

    Args:
        n_layers (`int`): The total number of layers.
        devices (`list`): A list of available devices.

    Returns:
        `dict`: A dictionary mapping layer indices to devices.
    """
    if len(devices) == 0:
        return {i: "cpu" for i in range(n_layers)}
    if len(devices) == 1:
        return {i: devices[0] for i in range(n_layers)}

    # Balanced distribution of layers across devices
    ...
    for device_idx, device in enumerate(devices):
        # Give one extra layer to first 'remainder' devices
        n_layers_on_device = layers_per_device + (1 if device_idx < remainder else 0)
        for _ in range(n_layers_on_device):
...
def load_sharded_checkpoint(model, folder, strict=True, prefer_safe=True):
    ...
    loader = (
        safe_load_file
        if load_safe
        else partial(torch.load, map_location="cpu", **weights_only_kwarg)
    )
...
def _load_state_dict_into_meta_model(
    ...
    for param_name, param in state_dict.items():
        ...
        is_param_float8_e4m3fn = (
            is_torch_e4m3fn_available and param.dtype == torch.float8_e4m3fn
        )
        if (
            dtype is not None
            and torch.is_floating_point(param)
            and not is_param_float8_e4m3fn
        ):
            if (
...
class PreTrainedModel(
    ...
    def _autoset_attn_implementation(
        ...
        if (
            ...
            if not isinstance(
                config._attn_implementation, dict
            ) and config._attn_implementation not in [
                "eager",
                "sdpa",
                "flash_attention_2",
            ]:
    ...
    def _check_and_enable_flash_attn_2(
        ...
        if (
            ...
            and torch.empty(0).device.type != "cuda"
        ):
...
def expand_device_map(device_map, param_names, start_prefix):
    ...
    new_device_map = {}
    param_names = [
        p[len(start_prefix) :] for p in param_names if p.startswith(start_prefix)
    ]
...
def get_disk_only_shard_files(device_map, sharded_metadata, start_prefix):
    ...
    weight_map = {
        p[len(start_prefix) :]: v
        for p, v in sharded_metadata["weight_map"].items()
        if p.startswith(start_prefix)
    }
