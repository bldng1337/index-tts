...
from .transformers_beam_search import (
    BeamScorer,
    BeamSearchScorer,
...
def isin_mps_friendly(elements, test_elements) -> torch.Tensor:
    """
    MPS-friendly version of torch.isin.

    This function was removed from transformers 5.x pytorch_utils.
    It handles MPS device compatibility issues with torch.isin.
    """
    if elements.device.type == "mps":
        # MPS doesn't support isin directly, move to CPU and back
        return torch.isin(elements.cpu(), test_elements.cpu()).to(elements.device)
    return torch.isin(elements, test_elements)


# Helper functions for assisted decoding (removed from transformers 5.x)
class HammingDiversityLogitsProcessor:
    """
    LogitsProcessor that enforces diversity in beam search by penalizing tokens
    that have already been selected in other beams of the same group.

    This class was removed from transformers 5.x and is provided here for
    backward compatibility with the project's diverse beam search implementation.

    Args:
        diversity_penalty (`float`):
            The penalty applied to tokens that have already been selected in other beams.
        num_beams (`int`):
            Number of beams.
        num_beam_groups (`int`):
    ...
    def __init__(self, diversity_penalty: float, num_beams: int, num_beam_groups: int):
        if not isinstance(diversity_penalty, float) and (
            isinstance(diversity_penalty, bool)
            or not isinstance(diversity_penalty, (int, float))
        ):
            raise ValueError("`diversity_penalty` should be a float")
    ...
    def __call__(
        ...
        group_start_idx = beam_group_idx * (self.num_beams // self.num_beam_groups)
        group_end_idx = min(
            group_start_idx + self.num_beams // self.num_beam_groups, self.num_beams
        )
        ...
        for batch_idx in range(batch_size):
            previous_group_tokens = current_tokens[
                batch_idx * self.num_beams : batch_idx * self.num_beams
                + group_start_idx
            ]
            group_current_tokens = current_tokens[
                batch_idx * self.num_beams + group_start_idx : batch_idx
                * self.num_beams
                + group_end_idx
            ]

            hamming_penalty = torch.zeros(
                scores.shape[1], dtype=torch.float, device=scores.device
            )
            for token in previous_group_tokens:
                hamming_penalty[token] = self.diversity_penalty

            for i in range(group_size):
                scores[batch_idx * self.num_beams + group_start_idx + i] -= (
                    hamming_penalty
                )
...
def _prepare_attention_mask(
    model_kwargs: dict, new_length: int, is_encoder_decoder: bool
) -> dict:
...
def _prepare_token_type_ids(model_kwargs: dict, new_length: int) -> dict:
...
def _crop_past_key_values(model, past_key_values, max_length: int):
    """Crops the past key values to the specified max length."""
    if past_key_values is None:
        return None

    # Handle Cache objects (DynamicCache, StaticCache, etc.)
    ...
    if isinstance(past_key_values, tuple):
        ...
        for layer_past in past_key_values:
            if layer_past is None:
                ...
                cropped_layer = tuple(
                    kv[:, :, :max_length, :] if kv is not None else None
                    for kv in layer_past
                )
                new_past.append(cropped_layer)
...
class GenerationMixin:
        ...
        if generation_config is None:
            ...
            if (
                ...
                and len(
                    ...
                    else self.config._get_generation_parameters()
                )
                > 0  # 3)
            ):
        ...
        if need_new_cache:
            ...
            if requires_cross_attention_cache:
                ...
                self._cache = EncoderDecoderCache(
                    self._cache, StaticCache(**encoder_kwargs)
                )
        else:
            self._cache.reset()
        return self._cache
    ...
    def _prepare_cache_for_generation(
        ...
        if generation_config.cache_implementation is not None:
            if (
                ...
                model_kwargs[cache_name] = QuantizedCache(
                    backend=backend, **cache_config
                )
    ...
    def _supports_num_logits_to_keep(self) -> bool:
        ...
        """
        return "num_logits_to_keep" in set(
    ...
    def generate(
        ...
        **kwargs,
        ...
        if (
            getattr(generation_config, "return_legacy_cache", False)
...
def _split_model_inputs(
    ...
    data_split_list = [
        {
            k: _split(model_input[k], full_batch_size, num_hidden_layers, split_size)[i]
...
def stack_model_outputs(
    ...
    def _concat(data):
        ...
        # New cache format
            ...
            return DynamicCache.from_batch_splits(
                data, num_hidden_layers=num_hidden_layers
            )
        elif isinstance(data[0], EncoderDecoderCache):
            return EncoderDecoderCache.from_batch_splits(
                data, num_hidden_layers=num_hidden_layers
            )
